...
public static class AdminEndpoints
{
    public static IEndpointRouteBuilder MapAdminEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        // ===== LiveKit 配置管理（仅 SuperAdmin） =====

        group.MapGet("/livekit-config", async (
            HttpContext ctx,
            ILiveKitConfigStore store,
            IOptions<LiveKitOptions> fallback,
            CancellationToken ct) =>
        {
            if (ctx.User.ToCurrentUser() is not { } cu)
                return Results.Unauthorized();
            // 仅 SuperAdmin 可查看 LiveKit 配置（含 Key）
            if (cu.Role != UserRole.SuperAdmin)
                return Results.Forbid();

            var cfg = await store.GetAsync(ct);
            bool fromDb = cfg is not null;
            if (cfg is null)
            {
                // 数据库尚未保存：返回 appsettings 兜底值，便于管理员编辑
                cfg = new LiveKitConfig
                {
                    Url = fallback.Value.Url,
                    ApiKey = fallback.Value.ApiKey,
                    ApiSecret = fallback.Value.ApiSecret,
                    UpdatedAt = DateTimeOffset.UtcNow,
                };
            }
            return Results.Ok(new LiveKitConfigResponse(
                cfg.Url,
                cfg.ApiKey,
                MaskSecret(cfg.ApiSecret),
                cfg.UpdatedAt,
                fromDb));
        }).RequireAuthorization(Policies.SuperAdminOnly);

        group.MapPut("/livekit-config", async (
            LiveKitConfigRequest req,
            HttpContext ctx,
            ILiveKitConfigStore store,
            CancellationToken ct) =>
        {
            ...
            if (string.IsNullOrWhiteSpace(req.Url))
                return Results.BadRequest(new { error = "Url 必填" });
            if (string.IsNullOrWhiteSpace(req.ApiKey))
                return Results.BadRequest(new { error = "ApiKey 必填" });

            var existing = await store.GetAsync(ct);

            // ApiSecret 为空串表示不修改
            if (string.IsNullOrEmpty(req.ApiSecret))
            {
                if (existing is null || string.IsNullOrWhiteSpace(existing.ApiSecret))
                    return Results.BadRequest(new { error = "首次配置必须提供 ApiSecret" });
            }

            var toSave = new LiveKitConfig
            {
                Url = req.Url.Trim(),
                ApiKey = req.ApiKey.Trim(),
                // 空串保留原值：由 SetAsync 的更新分支合并，此处给占位
                ApiSecret = string.IsNullOrEmpty(req.ApiSecret) ? "__KEEP__" : req.ApiSecret,
            };

            var saved = await store.SetAsync(toSave, ct);

            return Results.Ok(new LiveKitConfigResponse(
                saved.Url,
                saved.ApiKey,
                MaskSecret(saved.ApiSecret),
                saved.UpdatedAt,
                FromDatabase: true));
        }).RequireAuthorization(Policies.SuperAdminOnly);
    ...
    /// <summary>Secret 脱敏：首尾各保留 2 字符，中间以 * 填充。</summary>
    private static string MaskSecret(string secret)
    {
        if (string.IsNullOrEmpty(secret)) return string.Empty;
        if (secret.Length <= 4) return new string('*', secret.Length);
        return secret[..2] + new string('*', secret.Length - 4) + secret[^2..];
    }
